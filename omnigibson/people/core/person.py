...
class Person:
    ...
    people_asset_folder = setting_dict.get(
        PeopleSettings.CHARACTER_ASSETS_PATH)
    character_root_prim_path = setting_dict.get(
    ...
    def __init__(
        ...
        self.spawn_agent(self.char_usd_file,
                         self._stage_prefix, init_pos, init_yaw)
    ...
    def spawn_agent(self, usd_file, stage_name, init_pos, init_yaw):
        ...
        if not self._current_stage.GetPrimAtPath(Person.character_root_prim_path + "/Biped_Setup"):
            prim = prims.create_prim(Person.character_root_prim_path + "/Biped_Setup",
                                     "Xform", usd_path=Person.assets_root_path + "/Biped_Setup.usd")
    ...
    def add_animation_graph_to_agent(self):
        ...
        omni.kit.commands.execute("RemoveAnimationGraphAPICommand", paths=[
                                  Sdf.Path(self.character_skel_root.GetPrimPath())])

        # Add the animation graph to the character
        omni.kit.commands.execute("ApplyAnimationGraphAPICommand", paths=[Sdf.Path(
            self.character_skel_root.GetPrimPath())], animation_graph_path=Sdf.Path(animation_graph.GetPrimPath()))
    ...
    def get_character_asset_list():
        ...
        result, folder_list = omni.client.list(
            "{}/".format(Person.assets_root_path))
    ...
    def get_usd_in_folder(character_folder_path):
        ...
        if result != omni.client.Result.OK:
            carb.log_error("Unable to read character folder path at {}".format(
